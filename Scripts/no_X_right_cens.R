...
mod1 <- NULL
mod1 <- gamlss(y ~ 1, family=EXPrc, data=data,
               control=gamlss.control(trace=FALSE))
exp(coef(mod1))

mod2 <- NULL
mod2 <- gamlss(y ~ 1, family=LINrc, data=data,
               control=gamlss.control(trace=FALSE))
exp(coef(mod2))
...
parSim(
  ...
  name = "Simul/no_X_right_cens_part2",     # Name of the file
...
datos1 <- read.table("Simul/no_X_right_cens_part1.txt", header = TRUE)
datos2 <- read.table("Simul/no_X_right_cens_part2.txt", header = TRUE)
...
p0 <- ggplot(res00, aes(x=n, y=pcs, group=mu)) +
  ...
  ylim(0, 1) + 
  ggtitle("PCS for a random sample and 0% of right censored data")
...
p1 <- ggplot(res01, aes(x=n, y=pcs, group=mu)) +
  ...
  facet_wrap(~ dist, ncol = 2) + 
  ylim(0, 1) + 
  ggtitle("PCS for a random sample and 10% of right censored data")
...
p2 <- ggplot(res02, aes(x=n, y=pcs, group=mu)) +
  ...
  facet_wrap(~ dist, ncol = 2) + 
  ylim(0, 1) + 
  ggtitle("PCS for a random sample and 20% of right censored data")

p0
p1
p2

ggsave("Plots/no_X_right_cens_00.pdf", plot = p0, width = 8, height = 4)
ggsave("Plots/no_X_right_cens_01.pdf", plot = p1, width = 8, height = 4)
ggsave("Plots/no_X_right_cens_02.pdf", plot = p2, width = 8, height = 4)
