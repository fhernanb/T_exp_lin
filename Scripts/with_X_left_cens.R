...
data <- gen_data(mu=0.1, n=3000, dist="EXP", p_cens=0.0)

mod1 <- NULL
mod1 <- gamlss(y ~ x1 + x2, family=EXPlc, data=data,
               control=gamlss.control(trace=FALSE))
coef(mod1)

mod2 <- NULL
mod2 <- gamlss(y ~ x1 + x2, family=LINlc, data=data,
               control=gamlss.control(trace=FALSE))
coef(mod2)
...
parSim(
  ...
  name = "Simul/with_X_left_cens_part3", # Name of the file
...
datos1 <- read.table("Simul/with_X_left_cens_part1.txt", header = TRUE)
datos2 <- read.table("Simul/with_X_left_cens_part2.txt", header = TRUE)
datos3 <- read.table("Simul/with_X_left_cens_part3.txt", header = TRUE)
datos4 <- read.table("Simul/with_X_left_cens_part4.txt", header = TRUE)

datos <- rbind(datos1, datos2, datos3, datos4)
...
p0 <- ggplot(res00, aes(x=n, y=pcs, group=mu)) +
  ...
  ylim(0, 1) + 
  ggtitle("PCS for regression model and 0% of left censored data")
...
p1 <- ggplot(res01, aes(x=n, y=pcs, group=mu)) +
  ...
  facet_wrap(~ dist, ncol = 2) + 
  ylim(0, 1) + 
  ggtitle("PCS for regression model and 10% of left censored data")
...
p2 <- ggplot(res02, aes(x=n, y=pcs, group=mu)) +
  ...
  facet_wrap(~ dist, ncol = 2) + 
  ylim(0, 1) + 
  ggtitle("PCS for regression model and 20% of left censored data")

p0
p1
p2

ggsave("Plots/with_X_left_cens_00.pdf", plot = p0, width = 8, height = 4)
ggsave("Plots/with_X_left_cens_01.pdf", plot = p1, width = 8, height = 4)
ggsave("Plots/with_X_left_cens_02.pdf", plot = p2, width = 8, height = 4)
